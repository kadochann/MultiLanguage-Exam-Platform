...
namespace Project
{
    public partial class Soru : BasePage
    {
        ...
        private DataTable LoadQuestions()
        {
            int kategoriId = Session["KategoriId"] != null ? Convert.ToInt32(Session["KategoriId"]) : 1;
            string sql = "SELECT * FROM Sorular WHERE KategoriId = ?";
            return DbHelper.GetDataTable(sql, kategoriId);
        }
